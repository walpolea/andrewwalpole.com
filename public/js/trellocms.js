...
var TrelloCMS = function() {
    ...
    tcms.loadBoard = function(boardID, cb, options) {
    ...
    tcms.getBoardLabels = function() {
        if (!this.boarddata.labels) {
            return undefined;
        }

        return this.boarddata.labels;
    }

    tcms.getBoardLabelNames = function(color) {

        return this.getBoardLabels()
            .filter(function(labelObj) {
                if (color && labelObj.color !== color) {
                    return false;
                }
    ...
    tcms.getCardsByLabelName = function(labelName) {
        ...
        if (this.boarddata.cards) {

            for (var i = 0; i < this.boarddata.cards.length; i++) {
                if (this.cardHasLabel(this.boarddata.cards[i], labelName)) {
    ...
    tcms.searchCards = function(search) {
        ...
        if (this.boarddata.cards) {
            ...
            for (var i = 0; i < this.boarddata.cards.length; i++) {
                console.log(this.boarddata.cards[i].name, search);
                if (this.boarddata.cards[i].name.toLowerCase().indexOf(search.toLowerCase()) !== -1) {
                    c.push(this.boarddata.cards[i]);
                }
            }
        }
